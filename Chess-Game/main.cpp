...
int main(int argc, char* argv[]) {
	...
	while(!game_over) {
        ...
        // exit the program with return code -1 if an exception is caught here
				...
				if (!ifs) {//checking if file is successfully opened
					std::cerr << "Could not open file." << std::endl;
					ifs.close();
					return -1; //returning -1 if error is caught here
				}

				try{
					ifs >> game; //overloaded operator
					if (!game.is_valid_game()) {
						throw std::runtime_error("invalid game loaded");
					}
					ifs.close();
				}
				catch (const std::exception& e) { //catches any errors raised including those in add_piece and in overloaded >>
